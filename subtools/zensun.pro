...
if n_elements(dd) gt 1 then begin
  ...
  decang=eqtime
  ii=sort(dd)
  eqtime(ii)=spline(nday,eqt,dd(ii))/60.
  decang(ii)=spline(nday,dec,dd(ii))
endif else begin
  eqtime=spline(nday,eqt,dd)/60.
  decang=spline(nday,dec,dd)
endelse  
latsun=decang

if keyword_set(local) then begin
  tzone=fix((lon+7.5)/15)
  index = where(lon lt 0, cnt)
  if (cnt gt 0) then tzone(index) = fix((lon(index)-7.5)/15)
  ut=(time-tzone+24.) mod 24.                  ; universal time
  noon=tzone+12.-lon/15.                       ; local time of noon
endif else begin
  ut=time
